...
cd $TMPDIR/yaml-0.2.5/ && ./configure && make && sudo make install
...
cd $TMPDIR/PyYAML-5.3.1/ && python setup.py build && sudo python setup.py install


#### import setuptools
echoinfo "installing setuptools-40.0.0"
tar -zxf $_cdir/setuptools-40.0.0.tar.gz -C $TMPDIR
cd $TMPDIR/setuptools-40.0.0/ && python setup.py build && sudo python setup.py install


#### import jinja2
echoinfo "installing Jinja2-2.11.2.tar.gz"
tar -zxf $_cdir/Jinja2-2.11.2.tar.gz -C $TMPDIR
cd $TMPDIR/Jinja2-2.11.2/ && python setup.py build && sudo python setup.py install


#### import six.moves.urllib as urllib
echoinfo "installing six-1.15.0"
tar -zxf $_cdir/six-1.15.0.tar.gz -C $TMPDIR
cd $TMPDIR/six-1.15.0/ && python setup.py build && sudo python setup.py install


#### from pathlib2 import Path
echoinfo "installing scandir-1.10.0, pathlib2-2.3.5"
tar -zxf $_cdir/scandir-1.10.0.tar.gz -C $TMPDIR
cd $TMPDIR/scandir-1.10.0/ && python setup.py build && sudo python setup.py install

tar -zxf $_cdir/pathlib2-2.3.5.tar.gz -C $TMPDIR
cd $TMPDIR/pathlib2-2.3.5/ && python setup.py build && sudo python setup.py install


#### from markupsafe import Markup,escape
echoinfo "installing MarkupSafe-1.1.1"
tar -zxf $_cdir/MarkupSafe-1.1.1.tar.gz -C $TMPDIR
cd $TMPDIR/MarkupSafe-1.1.1/ && python setup.py build && sudo python setup.py install
